...
export default {
  ...
  created(){
    this._getData()
    console.log( this.getMDT(new Date().getTime()))
  ...
  methods: {
    ...
    _getData(){
      let option = {
        method: 'POST',
        url: `${BASEPATH_YUWEI}/info/personWarning/getPersonWarningChartData`,
        data: {
          userId: TEST_USER,
      ...
      this.$axios(option)
        .then(( data )=>{
          ...
          let leftTopData = data.personCountingMap
          let rightTopData = data.personChangesCountingMap
          let leftBottomData = data.personAreaGroupCountingMap
          let rightBottomData = data.personLevelCountingItemList
    ...
    uRightTopEchart(data){
      ...
      for (const id in data) {
        ...
        for (let i = 0, len = arr.length; i < len; i++) {
          let item = arr[i]
          seriesData.push([index, i, parseInt(item.timeBucketPersonNumber), areaGroupNameList[index]])// 添加层级圆圈的大小标识
        }
        index++
      }
    ...
    __createRBData(data){
      ...
      for (let i = 0, len = 20; i < len; i++) {
        data.push({
          ...
          personCount: Math.round(Math.random()*100),
        })
      }
      return data
    },
    ...
    uRightBottomEchart(data) {
      ...
      for (let i = 0, len = data.length; i < len; i++) {
        let item = data[i]
        let personCount = parseInt(item.personCount)
        max = max < personCount? personCount: max
      }
      // max = 1// gai
      ...
      for (let i = 0, len = data.length; i < len; i++) {
        let item = data[i]
        let value = parseInt(item.personCount)
    ...
    dataInitRightBottomEchart(data) {
      ...
      let option = {
        ...
        yAxis: {
          // max: max ? Math.round(max + max / 30) : 1,
    ...
    eToBack() {
      this.$router.push({ name: "warningOntime" });
    },
     //tip fn
