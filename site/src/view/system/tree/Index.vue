...
<script>
  ...
  import treeUtil from "../../../common/js/treeUtil.js";
  export default {
    data() {
      return {
        //filterText: '',
        ...
        return h(TreeItem,{
          ...
          on:{
            ...
            Append: () => {
              if(!this.editFlag){
                node.expanded = true
                data.children.push({ value: treeUtil.generateUUID(), label: '请输入模块名称', children: [],status:1,isAdd:true })
                this.editFlag = true
             }else{
                console.log(11111)
              }
           ...
           OpenEdit: (nodeData) => {
            ...
            Delete: (nodeData) => {
              //递归查找父节点
              var parentNode = treeUtil.getNode(this.treeData,data.value).parentNode
