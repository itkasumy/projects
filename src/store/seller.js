...
export default {
  ...
  getters: {
    logistics({sellerInfo}) {
      return !sellerInfo ? null : sellerInfo.logistics
    },
    limitDeliveryCost({sellerInfo}) {
      return !sellerInfo ? null : sellerInfo.limitDeliveryCost
    }
  },
  mutations: {
    ...
    setType(state, data) {
      state.sellerType = data
    }
  },
  actions: {
    // 首页广告
    async ajaxAdvertiseImg() {
      let {data: res} = await axios.post('/openservice/indexAD')
      return res
    },
    // 商家分类
